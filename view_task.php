...
if (isset($_GET['id'])) {
	$qry = $conn->query("SELECT t.*,concat(e.firstname,' ',e.lastname,' ',e.middlename) as name FROM task_list t inner join employee_list e on e.id = t.employee_id  where t.id = " . $_GET['id'])->fetch_array();
	foreach ($qry as $k => $v) {
		$$k = $v;
	}
}
  ...
  <div class="col-lg-12">
    <div class="row">
      <div class="col-md-6">
        ...
        <dl>
          <dt><b class="border-bottom border-primary">Employee Id</b></dt>
          <dd><?php echo ucwords($employee_id) ?></dd>
        </dl>
      </div>
      <div class="col-md-6">
        <dl>
