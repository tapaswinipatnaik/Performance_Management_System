...
<?php
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT *,concat(firstname,' ',lastname,' ',middlename) as name FROM employee_list where id = ".$_GET['id'])->fetch_array();
foreach($qry as $k => $v){
	$$k = $v;
}
...
$evaluator= $conn->query("SELECT *,concat(firstname,' ',lastname,' ',middlename) as name FROM evaluator_list where id = $evaluator_id ");
